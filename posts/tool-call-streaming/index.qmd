...
I got interested in this after reading Armin Ronacher’s post, [Pushing Local Models With Focus and Polish](https://lucumr.pocoo.org/2026/5/8/local-models/). One detail that stood out to me was tool-call streaming. It sounds minor, but it can make a big difference to how responsive an agent feels.
...
In this run, OpenAI returned the tool-call arguments in small fragments. Now let's see if the local engines match it.
...
For reproducibility, the deployment used the SGLang `v0.5.12.post1-cu130-runtime` image and pinned the Qwen model checkpoint to commit [`e89b16ebf1988b3d6befa7de50abc2d76f26eb09`](https://huggingface.co/Qwen/Qwen3.6-27B-FP8/commit/e89b16ebf1988b3d6befa7de50abc2d76f26eb09).
...
The entire `"content"` value arrived in **one chunk**. Unlike OpenAI's fine-grained fragments, it arrived in whole key/value pairs. SGLang is technically streaming (we got multiple chunks), but the granularity is dramatically coarser.
...
I was using llama.cpp version 9670 (`02810c7aa`). This time the arguments arrived in small fragments, much like OpenAI:
